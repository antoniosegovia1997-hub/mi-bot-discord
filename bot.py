...
# LEE DE RAILWAY - TIENES QUE TENER ESTAS 4 VARIABLES PUESTAS
...
async def publicar_lol(hora_utc, es_forzado=False):
    global inscritos, mensaje_ids, ultima_publicacion

    # Evita publicar 2 veces la misma hora
    hora_str = hora_utc.strftime("%Y-%m-%d-%H")
    if ultima_publicacion == hora_str and not es_forzado:
        return
    ultima_publicacion = hora_str

    # Reinicia inscripciones
    inscritos = {1: [], 2: [], 3: []}
    mensaje_ids = {1: None, 2: None, 3: None}
    ...
    for grupo, channel_id in [(1, C30_CHANNEL_ID), (2, C60_CHANNEL_ID), (3, C80_CHANNEL_ID)]:
        channel = client.get_channel(channel_id)
        if channel:
            titulo = f"🔄 PARTIDA {NOMBRES_TEAM[grupo]}" if es_forzado else f"🔥 NUEVA PARTIDA {NOMBRES_TEAM[grupo]}"
...
async def reloj_lol():
    ...
    # PUBLICAR CADA 2 HORAS EXACTAS: 00 02 04 06 08 10 12 14 16 18 20 22
    if now_es.minute == 0 and now_es.hour % 2 == 0:
        print(f"Disparando publicacion automatica: {now_es.strftime('%H:%M')} ES")
        await publicar_lol(now_es.astimezone(datetime.timezone.utc))

@client.command()
async def publicar(ctx, hora:str):
    """Forzar publicacion. Uso:!publicar 20:00"""
    try:
        h, m = map(int, hora.split(":"))
        now_es = datetime.datetime.now(TZ_ESPANA)
        hora_forzada = now_es.replace(hour=h, minute=m, second=0, microsecond=0)
        await publicar_lol(hora_forzada.astimezone(datetime.timezone.utc), es_forzado=True)
        await ctx.send(f"✅ Publicación forzada a las {hora} ES")
    except:
        await ctx.send("❌ Formato mal. Usa: `!publicar 20:00`")
...
async def on_interaction(interaction: discord.Interaction):
    if interaction.type == discord.InteractionType.component and interaction.data["custom_id"].startswith("grupo_"):
        ...
        # Quitar de otros grupos primero
        for g in inscritos:
        ...
        if user_id in inscritos[grupo]:
            ...
            await interaction.response.send_message(f"Saliste de {NOMBRES_TEAM[grupo]}", ephemeral=True)
        else:
